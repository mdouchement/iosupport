...
func (wc *WordCount) Perform() error {
  for wc.s.ScanLine() {
    ...
    if wc.Opts.CountByte { wc.Bytes += len(wc.s.Bytes()) + 1 } // +1 for EOL character
    if wc.Opts.CountChar { wc.Chars += len(wc.s.Text()) + 1 }  // idem
    if wc.Opts.CountWord { wc.Words += CountWords(wc.s.Text()) }
    if wc.Opts.CountLine { wc.Lines++ }
  }
  return nil
}
